...
class TableCanvas(Canvas):
    ...
    def __init__(self, parent=None, model=None, data=None, read_only=False,
                 width=None, height=None, icon=None,
    ...
    class StringDialog(simpledialog._QueryString):
        def body(pop, master):
            super().body(master)
            pop.iconbitmap(iconfile)
            
    class IntegerDialog(simpledialog._QueryInteger):
        def body(pop, master):
            super().body(master)
            pop.iconbitmap(iconfile)
                
    def ask_string(self, title, prompt, *args, **kargs):
        d = self.StringDialog(title, prompt, *args, **kargs)
        return d.result
                
    def ask_int(self, title, prompt, *args, **kargs):
        d = self.IntegerDialog(title, prompt, *args, **kargs)
        return d.result
    ...
    def addRows(self, num=None):
        ...
        if num == None:
            num = self.ask_int("How many rows?","Number of rows:",
                               initialvalue=1,parent=self.parentframe)
        if not num:
            return
    ...
    def autoAddColumns(self, numcols=None):
        ...
        if numcols == None:
            numcols = self.ask_int("Auto add rows.",
    ...
    def findValue(self, searchstring=None, findagain=None):
        ...
        if searchstring == None:
            searchstring = ask_string("Search table.",
                                               "Enter search value",
                                               parent=self.parentframe)
        found=0
    ...
    def showtablePrefs(self, prefs=None):
        ...
        self.prefswindow.iconbitmap(iconfile)
...
class ColumnHeader(Canvas):
    ...
    def relabel_Column(self):
        col=self.table.currentcol
        ans = ask_string("New column name?", "Enter new name:")
        if ans !=None:
            if ans == '':
